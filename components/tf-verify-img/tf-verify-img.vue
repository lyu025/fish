  ...
  <view class='tf-Box-Bg'>
    <view class='tf-Box'>
      <view class='tf-Box-title'>
        {{$t('common.pls_over_tf_verify')}}
        <view class='tf-close' @click.stop='close_'></view>
      </view>
      <canvas :style="{ width: canvasW + 'px', height: canvasH + 'px' }" class='tf-Box-center' canvas-id='tf-verify-canvas' id='tf-verify-canvas'></canvas>
      <movable-area class='tf-Box-BtnBox'>
        <view class='tf-Box-BtnBox-text'>{{$t('common.slide_slider')}}</view>
  ...
  methods:{
    ...
    endTouch(){
      if(Math.abs(this.canvasX-this.jgX)<=5){
        ...
        uni.showToast({
          title: this.$t('common.verify_failed'),
          icon:'error',
        });
        this.init();
      }
